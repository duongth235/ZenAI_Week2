...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == "train":
        ...
        # Step 8: update running mean/var
        ...
        #######################################################################
        # normalize bằng running statistics
...
def spatial_batchnorm_backward(dout, cache):
    ...
    # dùng backward của batchnorm thường
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    # Chia C channels thành G group
    x_group = x.reshape(N, G, C // G, H, W)

    # Normalize riêng cho từng sample n và từng group g
...
def spatial_groupnorm_backward(dout, cache):
    ...
    m = C // G * H * W  # số phần tử trong mỗi group

    # công thức backward giống layernorm
    dx_group = (1.0 / m) * (1 / np.sqrt(var + eps)) * (
        m * dx_hat_group
        - np.sum(dx_hat_group, axis=(2, 3, 4), keepdims=True)
        - x_hat_group * np.sum(dx_hat_group * x_hat_group, axis=(2, 3, 4), keepdims=True)
    )
